...
Feature: Проверка сайта emkworld.com
  ...
  Scenario Outline: emkworld.com заказать звонок
    ...
    When wait 2 sec
    When enter "generated_test_automation_email" in name field in "callback dialog section"
  ...
  Scenario Outline: emkworld.com оставить заявку
    ...
    When click on <button>
    When wait 2 sec
    When enter "generated_test_automation_email" in name field in "<section>"
  ...
  Scenario: emkworld.com contacts
    Given open emkworld.com page
    When enter "generated_test_automation_email" in name field in "contact section"
