...
Feature: Проверка сайта forging.emk24.su
  ...
  Scenario Outline: forging.emk24.su секция 1 и 2
    ...
    When enter "generated_name" in name field in "modal section"
    ...
    Then email with "generated_name" contains "[FORM]: Оставить заявку.;[PHONE]: +70000000000;[EMAIL]: automation.emk@gmail.com;[BODY]: Компания: test_automation_company"
    Examples:
      | button             | section   |
  ...
  Scenario: forging.emk24.su секция 3
    Given open forging.emk24.su page
    When enter "generated_name" in name field in "3 section"
    ...
    Then text "СПАСИБО ЗА ЗАЯВКУ!" is displayed
    Then email with "generated_name" contains "[FORM]: Форма заказа внизу;[PHONE]: +70000000000;[EMAIL]: automation.emk@gmail.com;[BODY]: Компания: test_automation_company"

  Scenario: forging.emk24.su заказать звонок
    Given open forging.emk24.su page
    When click on order call link
    When enter "generated_name" in name field in "order call section"
    When enter "+70000000000" in phone field in "order call section"
    When click on send button in "order call section"
    Then text "СПАСИБО ЗА ЗАЯВКУ!" is displayed
    Then email with "generated_name" contains "[FORM]: Заказать обратный звонок;[PHONE]: +70000000000"